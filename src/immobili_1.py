# Step 1 di progetto - Riporto da notebook
...
def parser_content(url):
    ...
    if r.status_code==200:
        ...
        if not ( len(tit) == len(prezzi_ann) and len(tit) == len(descr) and len(prezzi_ann) == len(descr) ):        
            print(f"Nella pagina sono stati trovati {len(prezzi_ann)} prezzi, {len(tit)} titoli, {len(descr)} descrizioni")
            print(f"ATTENZIONE: A causa di almeno un annuncio non completo, consigliamo visita 'manuale' della pagina: {url}")
